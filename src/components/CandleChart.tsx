...
export function CandleChart({ series, rangeDays, crosses, realizedPrice, balancedPrice, isDark }: Props) {
  ...
  useEffect(() => {
    ...
    const chart = createChart(container, {
      ...
      timeScale: { borderColor: isDark ? "#2e303a" : "#e5e4e7", timeVisible: false },
      autoSize: true,
    });
  ...
  useEffect(() => {
    ...
    const slice = (arr: (number | null)[]) => arr.slice(start);

    // 불마켓밴드: 20주 SMA와 21주 EMA 사이를 채운 밴드 (라방 차트의 갈색 띠).
    const bmsbSmaSlice = slice(series.bmsbSma);
    const bmsbEmaSlice = slice(series.bmsbEma);
    const bandData: BandData[] = [];
    for (let i = 0; i < dates.length; i++) {
      const a = bmsbSmaSlice[i];
      const b = bmsbEmaSlice[i];
    ...
    ma50.setData(lineData(dates, slice(series.ma50)));
    ...
    ma200.setData(lineData(dates, slice(series.ma200)));
    ...
    ma50w.setData(lineData(dates, slice(series.ma50w)));
    ...
    const markerApi = markers.length > 0 ? createSeriesMarkers(candleSeries, markers) : null;
    ...
    rsiSeries.setData(lineData(dates, slice(series.rsi14)));
    // 과매수 70 / 과매도 30 기준선
  ...
  }, [series, rangeDays, crosses, realizedPrice, balancedPrice, isDark]);
